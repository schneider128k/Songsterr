...
#
# IMPORTANT: lily names must be standard LilyPond drumPitchTable names.
# Custom aliases (cymca, hh, sn, bd etc.) are NOT in LilyPond's internal
# drumPitchTable and are silently dropped when rendering — even if they
# appear in a custom drumStyleTable. Always use canonical LilyPond names.
GM = {
    35: ('bassdrum',       2, 'Bass Drum 2'),
    36: ('bassdrum',       2, 'Bass Drum 1'),
    37: ('sidestick',      2, 'Side Stick'),
    38: ('acousticsnare',  2, 'Acoustic Snare'),
    39: ('acousticsnare',  2, 'Hand Clap'),
    40: ('electricsnare',  2, 'Electric Snare'),
    41: ('lowfloortom',    2, 'Low Floor Tom'),
    42: ('closedhihat',    1, 'Closed Hi-Hat'),
    43: ('highfloortom',   2, 'High Floor Tom'),
    44: ('pedalhihat',     2, 'Pedal Hi-Hat'),
    45: ('lowtom',         2, 'Low Tom'),
    46: ('openhihat',      1, 'Open Hi-Hat'),
    47: ('lowmidtom',      2, 'Low-Mid Tom'),
    48: ('himidtom',       2, 'High-Mid Tom'),
    49: ('crashcymbal',    1, 'Crash Cymbal 1'),
    50: ('hightom',        2, 'High Tom'),
    51: ('ridecymbal',     1, 'Ride Cymbal 1'),
    52: ('chinesecymbal',  1, 'Chinese Cymbal'),
    53: ('ridebell',       1, 'Ride Bell'),
    54: ('tambourine',     1, 'Tambourine'),
    55: ('splashcymbal',   1, 'Splash Cymbal'),
    56: ('cowbell',        1, 'Cowbell'),
    57: ('crashcymbalb',   1, 'Crash Cymbal 2'),
    58: ('vibraslap',      1, 'Vibraslap'),
    59: ('ridecymbal',     1, 'Ride Cymbal 2'),
    60: ('hightom',        2, 'Hi Bongo'),
    61: ('lowtom',         2, 'Low Bongo'),
    62: ('himidtom',       2, 'Mute Hi Conga'),
    63: ('hightom',        2, 'Open Hi Conga'),
    64: ('lowtom',         2, 'Low Conga'),
    65: ('himidtom',       2, 'High Timbale'),
    66: ('lowmidtom',      2, 'Low Timbale'),
    # Songsterr-specific extensions beyond standard GM
    97: ('crashcymbal',    1, 'Crash (alt)'),
    98: ('openhihat',      1, 'Open Hi-Hat (alt)'),
    92: ('halfopenhihat',  1, 'Half Open Hi-Hat'),
}


def _drum_note(midi: int, ghost=False, accent=0, tie=False):
    entry = GM.get(midi)
    if entry is None:
        return None
    lily, voice, _ = entry
    return DrumNote(midi=midi, lily=lily, voice=voice, ghost=ghost, accent=accent, tie=tie)
...
def parse_json(data: dict, title: str, artist: str) -> Score:
    ...
    # FIX: anacrusis — if the first measure has an 'anacrusis' field, its
    # notated duration is Fraction(anacrusis_value, 16) quarter-note beats,
    # not the full time-signature duration.  We read anacrusis in units of
    # 16th notes (the most common Songsterr encoding); adjust if you find
    # songs where the unit differs.
    cur_sig = [4, 4]
    meas_positions = []
    meas_durations = []   # actual notated duration per measure (may differ from sig for pickup)
    pos = Fraction(0)
    for mi, meas in enumerate(raw_measures):
        if 'signature' in meas:
            cur_sig = meas['signature']
        full_dur = Fraction(cur_sig[0], cur_sig[1])
        anacrusis = meas.get('anacrusis')
        if anacrusis is not None:
            # anacrusis = number of 16th-note beats in the pickup bar
            actual_dur = Fraction(int(anacrusis), 16)
        else:
            actual_dur = full_dur
        meas_positions.append(pos)
        meas_durations.append(actual_dur)
        pos += actual_dur
    ...
    for mi, raw in enumerate(raw_measures):
        ...
        while i < len(beats):
            ...
            # v8 grace note beat — duration does NOT advance the cursor
            if beat.get('graceNote'):
                ...
                if notes:
                    events.append(Event(
                        ...
                        notes=notes,
                        grace=True,
                        grace_is_v8=True,   # FIX: flag so emitter never advances cursor
                        grace_type=('on' if beat['graceNote'] == 'onBeat' else 'before')
                    ))
                i += 1
                continue

            # v5 flam: all real notes carry grace=true — duration DOES advance cursor
            ...
            if real and all(n.get('grace') for n in real):
                ...
                events.append(Event(
                    ...
                    notes=notes,
                    grace=True,
                    grace_is_v8=False,      # FIX: v5 flam — cursor does advance
                    grace_type='before',
